...
class AR_mapper():
    ...
    def plot_AR_synoptics(self, with_halo=True):
        ...
        ax1.text(-0.09,0.95,"(a)",fontsize=18,transform=ax1.transAxes)
        #calc_time=era5.hours[i]
        #---------------------------------------------------------------------#
        # Geographic data
        # Plot Geomap
        ...
        # Plot sea ice
        # Sea ice
        ax1.pcolormesh(self.seaice.lon, self.seaice.lat,
            np.array(self.seaice[:]), transform=ccrs.PlateCarree(), 
            cmap=self.reversed_map)
        # Create white overlay for less strong colors
        x=np.linspace(-90,90,41)
        y=np.linspace(50,90,93)
        x_grid,y_grid=np.meshgrid(x,y)
        white_overlay= np.zeros((41,93))+0.15
        ax1.contourf(x_grid,y_grid,white_overlay.T,
        ...
        C_ivt=ax1.contour(ds["longitude"],ds["latitude"],
                        ds[met_var_dict["ERA_name"][meteo_var]][hour,:,:],
                        levels=[100,500],linestyles="-",linewidths=1,
                        colors="darkblue",transform=ccrs.PlateCarree(),zorder=2)
        ...
        # mean sea ice cover
        ax1.contour(ds["longitude"],ds["latitude"],
                        ds["siconc"][hour,:,:]*100,levels=[15,85],
                        linestyles="-",linewidths=3,colors="white",
                        transform=ccrs.PlateCarree())
        ...
        ax2.text(-0.09,0.95,"(b)",fontsize=18,transform=ax2.transAxes) 
        ...
        # mean sea ice cover
        #ax2.contour(ds["longitude"],ds["latitude"],
        #                ds["siconc"][hour,:,:]*100,levels=[15,85],
        #                linestyles="-",linewidths=[3,3],color="white",
        #                transform=ccrs.PlateCarree())
        ...
        gls1.xlabel_style = {'size': 12}
        gls1.ylabel_style = {'size': 12}
        gls2.xlabel_style = {'size': 12}
        gls2.ylabel_style = {'size': 12}
...
class AR_IVT_tendency_plotter():
    ...
    def plot_sector_tendency(self):
        ...
        for k in range(4):
            ...
            C1=axis[k].contourf(era_ds["longitude"],era_ds["latitude"],
                ...
                zorder=6)
            C_ivt=axis[k].contour(era_ds["longitude"],era_ds["latitude"],
                era_ds[self.met_var_dict["ERA_name"][self.met_var]][era_index[col],:,:],
                levels=[100,500],linestyles="-",linewidths=2,
                colors="darkblue",transform=ccrs.PlateCarree(),zorder=8)
    ...
    def plot_both_flights(self,):
        ...
        for col in range(col_no):
            ...
            era_ds["IVT_u"]=era_ds["p71.162"]
            era_ds["IVT"]=np.sqrt(era_ds["IVT_u"]**2+era_ds["IVT_v"]**2)
            #era_ds["IVT"]=era_ds["IVT"].where(era_ds.IVT>99)
            # Make Grid but adapt it for specific subplot
            ...
            C1=axs[col].contourf(era_ds["longitude"],era_ds["latitude"],
                ...
                levels=self.met_var_dict["levels"][self.met_var],extend="max",
                transform=ccrs.PlateCarree(),
                cmap=self.met_var_dict["colormap"][self.met_var],alpha=0.9,
                zorder=6)
            C_ivt=axs[col].contour(era_ds["longitude"],era_ds["latitude"],
                era_ds[self.met_var_dict["ERA_name"][self.met_var]][era_index[col],:,:],
                levels=[100,500],lw=3,color="darkblue",
    ...
    def simplified_plot_both_flights(self,with_icon_box=False):
        ...
        col_no=len(flights)
        row_no=1
        fig,axs=plt.subplots(row_no,col_no,
            sharex=True,sharey=True,figsize=(12,16),
            subplot_kw={'projection': self.projection})
        key=0
        # Aircraft data
        fig_labels=["(a)","(b)"]
        ...
        for col in range(col_no):
            ...
            #----------------------------------------------------------------------#
            # Plot Geomap
            axs[col].add_image(self.stamen_terrain, 4)
            # Plot sea ice
            sea_ice_file_list=glob.glob(self.amsr2_sea_ice_path+\
                                        "*"+flight_date+"*.nc")
            sea_ice_ds=xr.open_dataset(sea_ice_file_list[0])
            ...
            C1=axs[col].contourf(era_ds["longitude"],era_ds["latitude"],
                ...
                levels=self.met_var_dict["levels"][self.met_var],extend="max",
                transform=ccrs.PlateCarree(),alpha=0.9,zorder=6,
                cmap=self.met_var_dict["colormap"][self.met_var])
            
            C1_c=axs[col].contour(era_ds["longitude"],era_ds["latitude"],
                era_ds[self.met_var_dict["ERA_name"][self.met_var]][era_index[0],:,:],
                levels=[self.met_var_dict["levels"][self.met_var][0],
                        self.met_var_dict["levels"][self.met_var][-1]],
                        color="purple",lw=3,transform=ccrs.PlateCarree())
            # Plot surface presure
            C_p=axs[col].contour(era_ds["longitude"],era_ds["latitude"],
                                    era_ds["msl"][era_index[0],:,:]/100,
                                    levels=np.linspace(950,1050,11),
            ...
            for sonde in sonde_data["launch_time"].keys():
                ...
                release_lon=sonde_data["reference_lon"][sonde].data[0]
                scat=axs[col].scatter(release_lon,release_lat,
                    marker=sonde_shapes[0],s=150,edgecolors="k",
                    c=sonde_data["IVT"].loc[sonde],cmap="BuGn",
                    vmin=0, vmax=500,transform=ccrs.PlateCarree(), zorder=40)
                #scat2=axs[col].scatter(release_lon,release_lat,
                #    marker=sonde_shapes[0],
                #           s=100,edgecolors="k",c=sonde_data["IWV"].loc[sonde],
                #           cmap="BuPu",vmin=0, vmax=20,
        ...
        cbar_ax = fig.add_axes([0.15, 0.3, 0.7, 0.01])
        cbar=fig.colorbar(C1, cax=cbar_ax,extend="max",orientation="horizontal")
        cbar.set_ticks([10,15,20])
        cbar_ax.text(0.4,-3.5,self.met_var+" "+\
                     self.met_var_dict["units"][self.met_var],
                     fontsize=18,transform=cbar_ax.transAxes)
        #cbar2_ax=fig.add_axes([0.6,0.28,0.1,0.01])
        #cbar2 = fig.colorbar(scat,cax=cbar_ax,extend="max",
        #                     orientation="horizontal")
        #cbar2.set_label("IVT ($\mathrm{kg}\mathrm{m}^{-1}\mathrm{s}^{-1}$)",
        #                fontsize=16)
            
        fig_path=os.getcwd()+"/../plots/"
        fig_name="Fig_AR_RF05_RF06_tendency.png"
        fig.savefig(fig_path+fig_name,dpi=300,bbox_inches="tight")
        print("Figure saved as:",fig_path+fig_name)
...
def run_plot_IVT_tendency(do_plot_tendency=True,
    ...
    met_var_dict["levels"]      = {"IWV":np.linspace(10,20,101),
    ...
    IVT_tendency_plotter=AR_IVT_tendency_plotter(config_dict,
                        do_plot_tendency=do_plot_tendency,
    ...
    if IVT_tendency_plotter.do_plot_tendency:
        ...
        IVT_tendency_plotter.simplified_plot_both_flights(with_icon_box=False)

if __name__=="__main__":
    run_plot_IVT_tendency(do_plot_tendency=False)